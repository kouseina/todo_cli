...
var command: [String] = [add, ls, rm, uncheck, check, toggle]
var data: [DataStruct] = []
...
repeat {
    ...
    if commandList.isEmpty {
        print("command list (add, ls, rm, check, uncheck, toggle, quit)")
        continue
    }
    ...
    case toggle:
        let split = input.components(separatedBy: "\(toggle) ")
        if split.count > 1 {
            let index : Int? = Int(split[1])
